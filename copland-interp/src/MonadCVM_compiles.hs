...
instance Prelude.Monad (St s) where
  return = ret
  (>>=) = bind
-}
--ret :: a -> St s a
...
inc_id =
  bind get (\st ->
    ...
    let {p' = st_pl st} in
    let {i = st_evid st} in
    bind (put (Mk_st e' tr' p' ((Prelude.+) i (Prelude.succ 0)))) (\_ ->
      ret i))
...
tag_ASP params mpl =
  bind inc_id (\x ->
    bind (add_tracem ((:) (Umeas x mpl params) ([]))) (\_ -> ret x))
  ...
  bind get_ev (\e ->
    bind get_pl (\p ->
      bind (tag_ASP params p) (\_ ->
        let {bs = do_asp params} in ret (cons_uu bs e params p))))
...
signEv =
  bind get_pl (\p ->
    bind get_ev (\e ->
      bind (tag_SIG p e) (\_ ->
        let {bs = do_sig (encodeEvBits e)} in ret (cons_sig bs e p))))
...
hashEv =
  bind get_pl (\p ->
    bind get_ev (\e ->
      bind (tag_HSH p e) (\_ ->
        let {bs = do_hash (encodeEvBits e)} in ret (cons_hh bs e p))))

copyEv :: CVM EvC
copyEv =
  bind get_pl (\p ->
    bind inc_id (\x -> bind (add_tracem ((:) (Copy x p) ([]))) (\_ -> get_ev)))
