...
code: do_sig do_asp encodeEvRaw Axioms_Io.cvm_events parallel_vm_thread
      Term_Defs.ASP_ID do_hash doRemote_session Term_Defs.Arg Term_Defs.BS
      Term_Defs.TARG_ID.
...
import qualified Data.ByteString as B (ByteString, concat, readFile)
import CryptoImpl (doHash, doSign)
import System.IO.Unsafe (unsafePerformIO)

--import CoplandLang_Extracted
--import IO_Stubs
...
data Ev =
   Copy Prelude.Int Plc
 | Umeas Prelude.Int Plc ASP_PARAMS
 | Sign Prelude.Int Plc Evidence
 | Hash Prelude.Int Plc Evidence
 | Req Prelude.Int Plc Plc Term Evidence
 | Rpy Prelude.Int Plc Plc Evidence
 | Split0 Prelude.Int Plc
 | Join Prelude.Int Plc
...
do_asp :: ASP_PARAMS -> BS
do_asp params = Prelude.undefined


lookupSecretKeyBytesIO :: Prelude.FilePath -> Prelude.IO BS
lookupSecretKeyBytesIO fp = do
  --fp <- asks myKeyPath
  bs <- B.readFile fp
  Prelude.return bs

get_key_simpl :: Prelude.IO BS
get_key_simpl = do
  --kp <- lookupSecretKeyPath
  let kp = "./key0.txt" in
    lookupSecretKeyBytesIO kp

do_sig_IO :: BS -> Prelude.IO BS
do_sig_IO bs = do
...
do_start_par_threadIO :: Loc -> Term -> RawEv -> Prelude.IO ()
do_start_par_threadIO loc t e = Prelude.undefined

doRemote_session :: Term -> Plc -> EvC -> EvC
doRemote_session t pTo e = Prelude.undefined

parallel_vm_thread :: Loc -> EvC
parallel_vm_thread l = Prelude.undefined
...
put_ev e =
  bind get (\st ->
    let {tr' = st_trace st} in let {p' = st_pl st} in let {i = st_evid st} in put (Mk_st e tr' p' i))
...
inc_id =
  bind get (\st ->
    ...
    let {e' = st_ev st} in
    let {p' = st_pl st} in
    let {i = st_evid st} in bind (put (Mk_st e' tr' p' ((Prelude.+) i (Prelude.succ 0)))) (\_ -> ret i))
  ...
  bind get_ev (\e ->
    bind get_pl (\p ->
      bind inc_id (\i ->
        ...
        let {e2 = splitEv_r sp e} in bind (add_tracem ((:) (Split0 i p) ([]))) (\_ -> ret ((,) e1 e2)))))

tag_ASP :: ASP_PARAMS -> Plc -> CVM Event_ID
tag_ASP params mpl =
  bind inc_id (\x -> bind (add_tracem ((:) (Umeas x mpl params) ([]))) (\_ -> ret x))

invoke_ASP :: ASP_PARAMS -> CVM EvC
invoke_ASP params =
  bind get_ev (\e ->
    bind get_pl (\p -> bind (tag_ASP params p) (\_ -> let {bs = do_asp params} in ret (cons_uu bs e params p))))

tag_SIG :: Plc -> EvC -> CVM Event_ID
tag_SIG p e =
  bind inc_id (\x -> bind (add_tracem ((:) (Sign x p (get_et e)) ([]))) (\_ -> ret x))

signEv :: CVM EvC
signEv =
  bind get_pl (\p ->
    bind get_ev (\e -> bind (tag_SIG p e) (\_ -> let {bs = do_sig (encodeEvBits e)} in ret (cons_sig bs e p))))

tag_HSH :: Plc -> EvC -> CVM Event_ID
tag_HSH p e =
  bind inc_id (\x -> bind (add_tracem ((:) (Hash x p (get_et e)) ([]))) (\_ -> ret x))

hashEv :: CVM EvC
hashEv =
  bind get_pl (\p ->
    bind get_ev (\e -> bind (tag_HSH p e) (\_ -> let {bs = do_hash (encodeEvBits e)} in ret (cons_hh bs e p))))

copyEv :: CVM EvC
copyEv =
  bind get_pl (\p -> bind inc_id (\x -> bind (add_tracem ((:) (Copy x p) ([]))) (\_ -> get_ev)))
...
event_id_span :: Term -> Prelude.Int
event_id_span t =
  case t of {
   Asp _ -> Prelude.succ 0;
   Att _ x -> (Prelude.+) (Prelude.succ (Prelude.succ 0)) (event_id_span x);
   Lseq x y -> (Prelude.+) (event_id_span x) (event_id_span y);
   Bseq _ x y -> (Prelude.+) ((Prelude.+) (Prelude.succ (Prelude.succ 0)) (event_id_span x)) (event_id_span y);
   Bpar _ x y -> (Prelude.+) ((Prelude.+) (Prelude.succ (Prelude.succ 0)) (event_id_span x)) (event_id_span y)}
...
inc_remote_event_ids t =
  bind get (\st ->
    ...
    let {e' = st_ev st} in
    let {p' = st_pl st} in
    let {i = st_evid st} in let {new_i = (Prelude.+) i (event_id_span t)} in put (Mk_st e' tr' p' new_i))
...
remote_session t p q e =
  bind (tag_REQ t p q e) (\_ ->
    ...
    bind (add_tracem (cvm_events t q (get_et e))) (\_ -> bind (inc_remote_event_ids t) (\_ -> ret e')))

doRemote :: Term -> Plc -> EvC -> CVM EvC
doRemote t q e =
  bind get_pl (\p -> bind (remote_session t p q e) (\e' -> bind (tag_RPY p q e') (\_ -> ret e')))
...
wait_par_thread loc t _ =
  bind get_pl (\_ ->
    bind (do_wait_par_thread loc) (\e' ->
      bind (add_tracem ((:) (Cvm_thread_end loc) ([]))) (\_ -> bind (inc_remote_event_ids t) (\_ -> ret e'))))
       ...
       (,) e1 e2 ->
        bind (put_ev e1) (\_ ->
          bind (copland_compile t1) (\_ ->
            bind get_ev (\e1r ->
              bind (put_ev e2) (\_ -> bind (copland_compile t2) (\_ -> bind get_ev (\e2r -> join_seq e1r e2r))))))});
      ...
      case pr of {
       (,) e1 e2 ->
        bind (start_par_thread loc t2 e2) (\_ ->
          bind (put_ev e1) (\_ ->
            bind (copland_compile t1) (\_ ->
              bind get_ev (\e1r -> bind (wait_par_thread loc t2 e2) (\e2r -> join_par e1r e2r)))))})}

run_cvm :: AnnoTermPar -> Cvm_st -> Prelude.IO Cvm_st
run_cvm t st =
